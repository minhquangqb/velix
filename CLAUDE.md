...
- 🟡 **Phase 3.5 — Webview UX (chèn thêm, làm TRƯỚC Phase 4)** — đang làm dần, ưu tiên Messenger chạy mượt trước. Lý do chèn: sau Phase 3 app "dùng được" về mặt UI nhưng chưa dùng thật được để chat. Phase 4 (plugin system) là tái cấu trúc, không cải thiện trải nghiệm; Downloads không liên quan nhắn tin.
  - ✅ **Notification Messenger — FIX rồi, user verify runtime OK** (commit tiếp theo). **Chẩn đoán quan trọng, ghi lại để khỏi lặp cho nền tảng sau**: Messenger web **KHÔNG gọi** `Notification`/`ServiceWorkerRegistration.showNotification`/`navigator.setAppBadge` — 3 hook của Phase 2 bắn trượt hết (`__velixProbe()` cho `ctor=0 sw=0 badge=0`). Lý do: **WebView2 báo `visibilityState: "visible"` cả khi webview bị hide**, nên Messenger tưởng user đang nhìn → tự nó không báo. Nguồn tin cậy duy nhất là **`document.title` dạng `"(N) Messenger"`**
    ...
    - `notify` command giờ nhận thêm `count: Option<u32>`: `Some` = set tuyệt đối (title biết chính xác), `None` = bump +1 (đường `Notification` API cũ). Nhờ vậy toast + badge không đá nhau, không cần thêm command mới vào ACL
    - Còn lại (chấp nhận cho bản này): toast chỉ ghi "Bạn có N tin nhắn mới", **chưa có tên người gửi + nội dung** — muốn thế phải đọc DOM Messenger = logic riêng nền tảng, để dành `inject.ts` plugin ở Phase 4. Dev mode toast vẫn mang tên PowerShell (caveat Phase 2, cần build+install mới đúng tên Velix)
    - **Chẩn đoán còn cắm trong code** (gỡ ở Phase 5 trước release): JS `window.__velixProbe()` (in trạng thái bridge) + `__velixTest()` (bắn toast thử, bỏ qua trang); Rust `eprintln!("[velix] ...")` trong `notify`/`set_badge` (chỉ debug build). `Ctrl+Shift+I` (debug build) mở devtools trên webview nền tảng đang hiện — phải là global shortcut vì webview con giữ bàn phím khi focus. Bộ này sẽ tái sử dụng khi bring-up Zalo/Telegram/ChatGPT: mỗi nền tảng phải `__velixProbe()` xem nó dùng API hay title
  ...
  - ⬜ **Link ngoài → trình duyệt hệ thống.** Bấm link trong chat làm webview điều hướng đi, mà ACL notification scope theo origin → rời host là mất quyền IPC, notification chết, không đường quay lại. Lỗi gặp ngay ngày đầu dùng
  - ⬜ **Navigation**: back/forward/reload + zoom native (`set_zoom`, **không** CSS transform — vỡ layout SPA)
